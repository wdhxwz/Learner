...
public class HttpRequestDecoder extends MessageDecoderAdapter {
    private static final char[] CONTENT_LENGTH = new String("Content-Length:").toCharArray();
        ...
        if (in.get(0) == (byte) 'G' && in.get(1) == (byte) 'E'
            ...
            for (int i = 0; i < last; i++) {
                ...
                for (int j = 0; j < CONTENT_LENGTH.length; j++) {
                    if (Character.compare(Character.toLowerCase((char)in.get(i+j)), Character.toLowerCase(CONTENT_LENGTH[j])) != 0) {
                        found = false;
                        break;
                    }
    ...
    private Map<String, String[]> parseRequest(StringReader is) {
        ...
        try {
            ...
            while ((line = rdr.readLine()) != null && line.length() > 0) {
                ...
                if (tokens[0].compareToIgnoreCase("Content-Length") == 0) {
